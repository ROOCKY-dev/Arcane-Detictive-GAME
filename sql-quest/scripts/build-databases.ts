/**
 * build-databases.ts
 * Generates .sqlite files from SQL seed data for all game locations.
 * Run with: npx tsx scripts/build-databases.ts
 *
 * Uses better-sqlite3 (build-time only, never shipped to the browser).
 */
...
const OUTPUT_DIR = join(process.cwd(), 'public', 'databases');
...
const databases: DbDefinition[] = [
  { name: 'archives_db', sql: ARCHIVES_SQL },
  { name: 'apothecary_db', sql: APOTHECARY_SQL },
  { name: 'beast_db', sql: BEAST_SQL },
  { name: 'underworld_db', sql: UNDERWORLD_SQL },
];

function buildDatabase({ name, sql }: DbDefinition): void {
  const outputPath = join(OUTPUT_DIR, `${name}.sqlite`);
  console.log(`Building ${name}.sqlite...`);

  // Remove existing file so we start fresh (avoid "table already exists" errors)
  ...
  try {
    // Disable FK enforcement: some seed data intentionally violates FK constraints
    // (e.g. seller_alias_id = 99 in underworld_db for the "Phantom Fence" quest)
...
console.log('=== SQL Quest: Building game databases ===\n');
